...
import seoConfig from "#de7dlkp6n0nx";
import { logger } from "#iekdnbhxbb0g";
import { LANGUAGE_ROUTING } from "#fwwb6s6610u5";
import { allRoutePaths } from "#ji6pz0psckkq";
...
async function writeSiteShell(
  ...
  const routes = createLocaleShellRoutes({
      ...
      strategy,
  }).map((route) => ({ ...route, body: `${route.body}${siteStructuredData(route.sourcePath, rootDir)}` }));
